...
uint64_t experimentalXDS(uint64_t const alpha,
						 uint64_t const beta,
	...
	for(uint64_t x = 0; x < bound_n; x++){
		for(uint64_t y = 0; y < bound_n; y++){
			if((((x+y)&mask_n) ^ (((x^alpha)+(y^beta))&mask_n)) == delta)
				ctr++;
		}
	}
	return ctr;
...
uint64_t experimentalXDSbar(uint64_t const alpha,
							uint64_t const beta,
							uint64_t const delta,
							uint const n){
	...
	for(uint64_t x = 0; x < bound_n; x++){
		for(uint64_t y = 0; y < bound_n; y++){
			if((((x+y)&mask_n) ^ (((x^alpha)+(y^beta)+1)&mask_n)) == delta)
				ctr++;
		}
	}
	return ctr;
...
uint64_t experimentalRn(uint64_t const alpha,
						uint64_t const beta,
						uint64_t const delta,
						uint const n){
	uint64_t bound_n = (1ULL << n);
	...
	for(uint64_t x = 0; x < bound_n; x++){
		for(uint64_t y = 0; y < bound_n; y++){
			if(((((x+y)&mask_n) ^ (((x^alpha)+(y^beta))&mask_n)) == delta) && (x+y < bound_n))
				ctr++;
		}
	}
	return ctr;
...
uint64_t experimentalRnbar(uint64_t const alpha,
						uint64_t const beta,
						uint64_t const delta,
						uint const n){
	uint64_t bound_n = (1ULL << n);
	...
	for(uint64_t x = 0; x < bound_n; x++){
		for(uint64_t y = 0; y < bound_n; y++){
			if(((((x+y)&mask_n) ^ (((x^alpha)+(y^beta)+1)&mask_n)) == delta) && (x+y < bound_n))
				ctr++;
		}
	}
	return ctr;
...
int main(){
	...
	GRBEnv env = GRBEnv();
	uint n = 32;
	for(uint rotationOffset = 1; rotationOffset < n; rotationOffset++){
...
*/
	...
	//alpha = 11111 beta = 11111 delta = 11100 experimental : 72 theoretical : 0
	// uint64_t alpha = 0b11111;
	// uint64_t beta  = 0b11111;
	// uint64_t delta = 0b11100;
	// uint n = 5;
	// uint k = 2;
	// getRXDiffCount(alpha,beta,delta,n,k);
	
	// for(uint n = 2; n < 7; n++){
	// 	for(uint k = 0; k < n; k++){
	// 		cout << "n = " << n << " k = " << k << endl;
	// 		checkCountFormula(n,k);
	// 	}
	// }
	...
	// cout << "XDS(a,b,d) = " << experimentalXDS(alpha,beta,delta,n) << endl;
	// cout << "XDS(~a,~b,~d) = " << experimentalXDS(alpha^mask_n,beta^mask_n,delta^mask_n,n) << endl;
	// cout << "~XDS(a,b,d) = " << experimentalXDSbar(alpha,beta,delta,n) << endl;

	// cout << "Rn(a,b,d) = " << experimentalRn(alpha,beta,delta,n) << endl;
	// cout << "Rn(~a,~b,~d) = " << experimentalRn(alpha^mask_n,beta^mask_n,delta^mask_n,n) << endl;
	// cout << "~Rn(a,b,d) = " << experimentalRnbar(alpha,beta,delta,n) << endl;
}