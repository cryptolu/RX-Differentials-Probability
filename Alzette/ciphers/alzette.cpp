...
knownTrail can be provided as a starting solution for the model
...
*/
	...
	char cipher_name[4096] = {};
	sprintf(cipher_name, "Alzette_%08lx_k%d_nr%d", cst, gamma, nbRound);
	...
	auto const & trail = knownTrail.stateWords;
    if(trail.size() > 0){
    	assert(knownTrail.wordSize == 32);
    	assert(knownTrail.nbStateWords == 2);
    	assert(knownTrail.nbKeyWords == 0);
		...
		assert(trail.size() == nbRound+1);
		for(uint r = 0; r < nbRound+1; r++){
			auto const trail_r = trail[r];
			assert(trail_r.size() == 2);
			for(uint i = 0; i < n; i++){
	...
	int focus = -1;
	int cuts = -1;
	double heuristics = -0.1;
	printf("\n\n");
	printf("------------------------\n");
	printf("FOCUS = %d HEURISTICS = %lf CUTS = %d\n", focus, heuristics, cuts);
	printf("------------------------\n");
	printf("\n\n");
	...
	if (heuristics >= 0.0)
		model.set(GRB_DoubleParam_Heuristics, heuristics);

	if (cuts != -1)
		model.set(GRB_IntParam_Cuts, cuts);
	
	model.update();
	model.optimize();
	if(model.get(GRB_IntAttr_SolCount) > 0){
		Trail retTrail = {};
		retTrail.cipher = string(cipher_name);
		retTrail.wordSize = 32;
		assert(n == 32);
		retTrail.rotationOffset = gamma;
		...
		for(uint i = 0; i < nbRound; i++){
			...
			retTrail.modAddWords.push_back({valx[i], valy[i]});
		}
...
//Create a MILP model for Alzette to find a plaintext pair matching a given trail
    ...
    assert(trail.stateWords.size() == nbRound + 1);
    for(uint r = 0; r < nbRound+1; r++){
	    assert(trail.stateWords[r].size() == 2);
    	addValueToBinVectorConstr(model,x[r],trail.stateWords[r][0]);
    	addValueToBinVectorConstr(model,y[r],trail.stateWords[r][1]);
    }
...
//Return a pair {b, p}, b = true if a plaintext pair was found (and then p contains the pair as x0,y0,x1,y1)
	...
	if(model.get(GRB_IntAttr_SolCount) == 0){
		...
		cout << "0x" << setfill('0') << setw(n/4) << hex << trail.stateWords[0][0] << " ";
		cout << "0x" << setfill('0') << setw(n/4) << hex << trail.stateWords[0][1] << endl;
		...
		cout << "0x" << setfill('0') << setw(n/4) << hex << trail.stateWords[nbRound][0] << " ";
		cout << "0x" << setfill('0') << setw(n/4) << hex << trail.stateWords[nbRound][1] << endl;

		if(dx == trail.stateWords[0][0] && dy == trail.stateWords[0][1])
			cout << "Input differentials matches" << endl;
		else
			cout << "Input differentials fail" << endl;
		if(cdx == trail.stateWords[nbRound][0] && cdy == trail.stateWords[nbRound][1])
